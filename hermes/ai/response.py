...
class ResponseChunk:
    ...
    def is_final(self) -> bool:
        ...
        return self.finish_reason is not None or (self.usage is not None and self.usage.total_tokens > 0)
...
class ResponseValidator:
    ...
    def validate_response(response: AIResponse) -> None:
        ...
        if not response.success and not response.message:
            raise ValueError(
                "An error response must have a message."
            )
    ...
    def validate_choice(choice: ResponseChoice) -> None:
        ...
        if not choice.message.content and not choice.message.tool_call_id:
            raise ValueError(
                "A choice must have either content or a tool_call_id."
            )
...
class ResponseSerializer:
    ...
    def from_dict(data: dict[str, Any]) -> AIResponse:
        ...
        function_calls = [FunctionCall.from_dict(f) for f in data.get("function_calls", [])]
        usage = None
