...
from typing import Any, Callable, Iterable, Self, MutableSequence
...
class ConversationHistory:
    ...
    def search(self, text: str, case_sensitive: bool = False) -> list[ConversationMessage]:
    ...
    def filter(
        ...
        if role is not None:
            ...
            result = [m for m in result if (m.role.value if isinstance(m.role, PromptRole) else m.role) == role_str]
    ...
    def slice(self, start: int | None = None, end: int | None = None) -> list[ConversationMessage]:
...
class ConversationValidator:
    ...
    def validate_message(message: ConversationMessage) -> None:
        ...
        if not message.content and not message.tool_calls and not message.function_calls:
            raise ValueError("Message must have content, tool_calls, or function_calls.")
    ...
    def validate_state_transition(current: ConversationState, new: ConversationState) -> None:
        ...
        """
        allowed = {
            ConversationState.ACTIVE: {ConversationState.PAUSED, ConversationState.ARCHIVED, ConversationState.CLOSED},
            ConversationState.PAUSED: {ConversationState.ACTIVE, ConversationState.ARCHIVED, ConversationState.CLOSED},
            ConversationState.ARCHIVED: {ConversationState.ACTIVE, ConversationState.CLOSED},
...
class ConversationSerializer:
    ...
    def to_dict(conversation: AIConversation) -> dict[str, Any]:
        ...
        return {
            ...
            "summary": conversation._summary.to_dict() if conversation._summary else None,
    ...
    def from_dict(data: dict[str, Any]) -> AIConversation:
        ...
        conv._messages = [ConversationMessage.from_dict(m) for m in data.get("messages", [])]
        conv._branches = [ConversationBranch.from_dict(b) for b in data.get("branches", [])]
        conv._checkpoints = [ConversationCheckpoint.from_dict(cp) for cp in data.get("checkpoints", [])]
        if data.get("summary"):
    ...
    def to_json(conversation: AIConversation) -> str:
        """Serialize to JSON."""
        return json.dumps(ConversationSerializer.to_dict(conversation), indent=2, ensure_ascii=False, default=str)
        ...
        for response in session.history.responses:
            if response.choices:
                for choice in response.choices:
                    conv.add_message(role=choice.message.role, content=choice.message.content)
        return conv
...
class AIConversation:
    ...
    def off(self, event_type: str, callback: Callable) -> None:
        ...
        if event_type in self._event_listeners:
            self._event_listeners[event_type] = [cb for cb in self._event_listeners[event_type] if cb != callback]
    ...
    def tool(self, content: str, tool_call_id: str, name: str | None = None, **kwargs) -> Self:
        """Add a tool message."""
    ...
    def append_delta(self, delta: str) -> None:
        ...
        if msg is None:
            raise ValueError(f"Streaming message {self._streaming_message_id} not found.")
        msg.content += delta
        self._streaming_accumulated += delta
        self._touch()
        self._emit_event("stream_delta", {"message_id": self._streaming_message_id, "delta": delta})
    ...
    def finish_stream(self, finish_reason: str = "stop") -> str:
        ...
        self._streaming_accumulated = ""
        self._touch()
        self._emit_event("stream_finished", {"message_id": msg.id, "finish_reason": finish_reason})
        return msg.id
    ...
    def generate_summary(self, token_limit: int = 200) -> Self:
        ...
        if len(text) > 500:
            text = text[:500] + "..."
        self._summary = ConversationSummary(content=text, tokens=self.estimate_tokens() // 2)
        self._touch()
    ...
    def clone(self) -> AIConversation:
        ...
        for msg in self._messages:
            ...
            new_msg.children = [id_map.get(cid, cid) for cid in msg.children if cid in id_map]
