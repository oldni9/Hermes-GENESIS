...
class NetworkAIProvider(BaseAIProvider, ABC):
    ...
    def validate(self) -> None:
        ...
        if not self._base_url:
            raise ValueError(f"{self.metadata.name}: base_url is required.")
    ...
    def _request(
        ...
        *,
        ...
        if not self._started:
            raise RuntimeError(f"{self.metadata.name}: provider not started.")

        if self._client is None:
            raise RuntimeError(f"{self.metadata.name}: HTTP client is None.")
        ...
        raise ProviderError(f"{self.metadata.name}: request failed.") from last_error
