...
from hermes.ai.conversation import (
    AIConversation,
    ConversationMessage,
)
...
class Chat:
    ...
    def _build_context(self) -> AIContext:
        ...
        Includes conversation and session data.
        """
        return AIContext(
    ...
    def send(
        ...
        # Emit event
        self._emit_event(
            "message_sent", {"message": message, "request": request.to_dict()}
        )
    ...
    def stream(
        ...
        # Begin stream message – provide non‑empty content to satisfy validation
        self._streaming_message_id = self._conversation.begin_stream(
            assistant_message=" ",  # placeholder to satisfy validation
            parent_id=None,
        )
        provider_name = self._resolve_provider(request)
        ...
        try:
            if self._pipeline is not None:
                # Pipeline streaming path (mirrors send() pipeline usage)
                context = self._build_context()
                stream_gen = self._pipeline.stream(
                    provider_name,
                    request,
                    context,
                    use_cache=False,
                ...
                try:
                    while True:
                        ...
                        if (
                            ...
                            and chunk.choices[0].message.content
                        ):
                            self._conversation.append_delta(
                                chunk.choices[0].message.content
                            )
                except StopIteration as e:
                    final_response = e.value
                except GeneratorExit:
                    # Propagate close to inner generator
                    try:
                        stream_gen.close()
                    finally:
                        raise
            else:
                # Fallback: manager-based simulation (existing behavior)
                response = self._manager.execute(
                    provider_name,
                    request,
                    context=self._session,
                ...
                if (
                    ...
                    self._conversation.append_delta(chunk.choices[0].message.content)
            ...
            if final_response is not None:
                ...
                self._emit_event(
                    "response_received", {"response": final_response.to_dict()}
                )
            elif self._streaming_message_id is not None:
                # Stream was cancelled (e.g., generator closed) without returning
                self._conversation.cancel_stream()
    ...
    def retry(self, options: dict[str, Any] | None = None) -> AIResponse:
        ...
        return self.send(
            user_msg.content, system=user_msg.metadata.get("system"), options=options
        )
    ...
    def export_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "model": self._model,
            "last_request": (
                self._last_request.to_dict() if self._last_request else None
            ),
            "last_response": (
    ...
    @classmethod
    def import_dict(
        cls, data: dict[str, Any], manager: AIManager | None = None
    ) -> Chat:
        """
        ...
        data : dict[str, Any]
            Dictionary representation.
        manager : AIManager | None, optional
            AI Manager instance (deprecated).
...
# ✓ Methods:
#   - __init__ (with pipeline, manager deprecated)
#   - set_provider / set_model / set_config
#   - on / off
#   - send (uses pipeline)
#   - stream (uses pipeline if available, with proper generator handling)
#   - retry
#   - clear_history / reset
...
# ✓ Compatibility:
#   - Works with AIConversation, AISession, AIRequest, AIResponse, AIManager, AIPipeline
#   - Future-compatible with Memory and Tool systems

# ✓ Changes made in this PR:
#   - Only the `stream()` method has been modified.
#   - Uses `_resolve_provider()` to resolve provider (consistent with `send()`).
#   - Added pipeline path that iterates `pipeline.stream()` and yields chunks.
#   - Appends deltas to conversation.
#   - Captures final response from `StopIteration.value`.
#   - Propagates `GeneratorExit` to inner generator if outer generator is closed.
#   - Fallback to existing manager simulation remains identical.
