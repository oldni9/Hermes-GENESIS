...
class SessionHistory:
    ...
    def __iter__(
        ...
        tuple[AIRequest | None, AIResponse | None, Prompt | None, SessionEvent | None]
    ]:
        """
...
class AISession:
    ...
    def _transition_to(self, new_state: SessionState) -> None:
        ...
        self._touch()
        self._record_event(
            "state_change", {"from": self._state.value, "to": new_state.value}
        )
    ...
    def add_request(self, request: AIRequest) -> Self:
        ...
        self._record_event(
            "request_added", {"request_id": getattr(request, "id", None)}
        )
        self._touch()
        return self
    ...
    def clone(self) -> AISession:
        ...
        new_session._history._requests = [
            deepcopy(req) for req in self._history._requests
        ]
        new_session._history._responses = [
            deepcopy(resp) for resp in self._history._responses
        ]
        new_session._history._prompts = [
            deepcopy(prompt) for prompt in self._history._prompts
        ]
        new_session._history._events = [
            deepcopy(event) for event in self._history._events
        ]
    ...
    def import_dict(cls, data: dict[str, Any]) -> AISession:
        ...
        session._statistics = SessionStatistics(
            ...
            created_at=datetime.fromisoformat(
                stats_data.get("created_at", datetime.utcnow().isoformat())
            ),
        )
...
class SessionFactory:
    ...
    def from_request(request: AIRequest) -> AISession:
        ...
        """
        session = AISession(
            provider=request.provider or "default", model=request.model or "default"
        )
        session.add_request(request)
        return session
    ...
    @staticmethod
    def from_prompt(
        prompt: Prompt, provider: str | None = None, model: str | None = None
    ) -> AISession:
        """
