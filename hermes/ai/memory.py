...
class MemoryStore:
    ...
    def update(
        self, entry_id: str, content: str | None = None, importance: float | None = None
    ) -> bool:
        """
    ...
    def _prune(self) -> None:
        ...
        """
        short_term = [
            e for e in self._entries.values() if e.memory_type == MemoryType.SHORT_TERM
        ]
        long_term = [
            e for e in self._entries.values() if e.memory_type == MemoryType.LONG_TERM
        ]
    ...
    def statistics(self) -> MemoryStatistics:
        ...
        stats.short_term_count = sum(
            1 for e in entries if e.memory_type == MemoryType.SHORT_TERM
        )
        ...
        stats.average_importance = (
            sum(e.importance for e in entries) / stats.total_entries
        )
...
class Memory:
    ...
    def search(
        ...
        return self._store.search(
            query, memory_type, tags, min_importance, max_importance, limit
        )
    ...
    def filter(
        ...
        return self._store.filter(
            memory_type, tags, min_importance, max_importance, start_time, end_time
        )
