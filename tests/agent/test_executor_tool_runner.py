...
def test_tool_runner_passes_context(tool_runner, mock_tool_manager):
    """Verify that the provided ToolContext is passed to execute_batch."""
    tc1 = make_provider_tool_call("c1", "search")
    mock_tool_manager.execute_batch.return_value = [
        ToolResult(call_id="c1", status=ToolStatus.SUCCESS, output="Result")
    ]
    
    # FIX: ToolContext uses `session`, not `session_id`
    context = ToolContext(session="test-session-123")
    tool_runner.execute([tc1], context=context)
    
    # Verify the context was passed down
