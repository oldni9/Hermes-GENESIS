...
def test_pipeline_middleware_error_after(mock_orchestrator: MagicMock) -> None:
    ...
    pipeline = AIPipeline(
        orchestrator=mock_orchestrator, middlewares=[ErrorAfterMiddleware()]
    )
    request = AIRequest(prompt="hello")
...
def test_pipeline_cache_still_works_with_middleware(
    mock_orchestrator: MagicMock,
) -> None:
    ...
    pipeline = AIPipeline(
        orchestrator=mock_orchestrator, cache=cache, middlewares=[LoggingMiddleware()]
    )
    request = AIRequest(prompt="hello")
...
def test_pipeline_cache_hit_skips_middleware(mock_orchestrator: MagicMock) -> None:
    ...
    cached_response = AIResponse(
        success=True, result="cached", provider="test", model="test"
    )
    cache = MagicMock(spec=AICache)
